...
public class ExcelUtils {
    ...
    public static Workbook getWorkbookAndVerify(MultipartFile file) {
        Workbook workbook = getWorkbook(file);
        if (workbook == null) {
            throw new ApiException("获取excel文件出错！");
        }
        if (workbook.getNumberOfSheets() <= 0) {
            throw new ApiException("excel文件没有数据！");
        }
        return workbook;
    }

    public static void handleDataFormat(Sheet sheet) {
        // 处理数据格式
     ...
     * @param tableHeads 字段名，“-”符号分隔
     * @return List<List < String>>
     */
    public static List<List<String>> getExcelData(MultipartFile file, String tableHeads) {
        if (file.isEmpty()) {
            throw new ApiException("请选择导入文件！");
        }
        // 表格数据
        List<String> columNames = Arrays.asList(tableHeads.split("-"));
        IOExcelUtils.SheetData sheetData = IOExcelUtils.validExcel(file, columNames);
        return sheetData.getDatas();
    }

    /**
     * 默认方式
     * 导出导入模板
     * p：使用默认导出模板 而没有使用ExcelUtils.getExcelData读取数据 则需要判断不为空的行才算是有效数据
     *
     * @param tableHeads      字段名，“-”符号分隔
     ...
     */
    public static void defDownloadModule(String tableHeads,
                                         String excelName,
                                         HttpServletRequest servletRequest,
                                         HttpServletResponse servletResponse) {
        try {
            final int maxRow = 999;
            List<String> titleName = Arrays.asList(tableHeads.split("-"));
            Workbook wb = ExcelUtils.createWorkBook(excelName, (String) null, null, titleName);
            ...
            if ("IE".equals(ExcelUtils.getBrowser(servletRequest))) {
    ...
    /**
     * @param sysSource 资源文件
     * @return org.apache.poi.ss.usermodel.Workbook poi工作簿
     * @author leihfei
     * @description 获取MultipartFile中的文件路径及文件Input流
     * @date 10:28 2019/2/14
     */
//    public static Workbook getWorkbook(SysSource sysSource) {
//        String ext = "";
//        ext = ext.toLowerCase();
//        String fpath = sysSource.getFileOriginalName();   //取得文件路径
//        if (fpath.lastIndexOf(".") > -1) {
//            ext = fpath.substring(fpath.lastIndexOf(".") + 1);//获取后缀名
//        }
//        try {
//            String filePath = sysSource.getFileSource() + "/" + sysSource.getFileNewName();
//            InputStream is = new FileInputStream(filePath);  //转为Input流
//            return getWorkbook(is, ext);
//        } catch (IOException e) {
//            e.printStackTrace();
//            return null;
...
//    }
    ...
    public static void mergeCell(Sheet sheet, Integer firstRow, Integer lastRow, Integer firstCellNum, Integer lastCellNum) {
        CellRangeAddress region = new CellRangeAddress(firstRow, lastRow, firstCellNum, lastCellNum);
        sheet.addMergedRegion(region);
        RegionUtil.setBorderLeft(BorderStyle.NONE, region, sheet);
        RegionUtil.setBorderTop(BorderStyle.NONE, region, sheet);
        RegionUtil.setBorderRight(BorderStyle.NONE, region, sheet);
        RegionUtil.setBorderBottom(BorderStyle.NONE, region, sheet);
    }
     ...
     * @date 15:45 2019/2/21
     */
    public static void mergeCellBorder(Sheet sheet, Integer firstRow, Integer lastRow, Integer firstCellNum, Integer lastCellNum) {
        CellRangeAddress region = new CellRangeAddress(firstRow, lastRow, firstCellNum, lastCellNum);
        sheet.addMergedRegion(region);
    ...
    public static void downloadExcel(String location, String fileName, String fileRealName
        ...
        try {
            ...
            e.printStackTrace();
            throw new ApiException("Excel导出异常！");
        } finally {
            if (inputStream != null) {
                try {
                    inputStream.close();
     ...
     * 校验导入的Excel数据
     *
     * @param sysSource   - 导入的文件
     * @param columnNames - 模板表头
     * @return 返回表格数据
     */
//    public static SheetData validExcel(SysSource sysSource, List<String> columnNames) {
//        // 读取Excel数据
//        List<SheetData> sheetDatas = readExcel(sysSource);
//        SheetData sheetData = null;
//        if (CollectionUtils.isNotEmpty(sheetDatas)) {
//            sheetData = sheetDatas.get(0);
//        }
//        if (sheetData == null) {
//            throw new ApiException("Excel读取错误！");
//        }
//        List<String> expNames = sheetData.getColumnNames();
//        if (columnNames.size() != expNames.size()) {
//            throw new ApiException("请按模板填写数据!");
//        }
//        //校验表头
...
//    }
     ...
     * @return
     */
//    public static List<SheetData> readExcel(SysSource sysSource) {
//        //读取工作簿
//        Workbook workBook = getWorkbook(sysSource);
//        if (workBook == null) {
//            throw new ApiException("Excel解析错误！");
//        }
...
//    }
    ...
    public static List<SheetData> readExcel(MultipartFile file) {
